...
function Message:Init( container, max_at_the_same_time )
	print('Message.lua[Init]')
	assert(container, "Message:Init() : container cannot be nil!")
	self.msg_list = {}
	self.container = container
	self.max_at_the_same_time = max_at_the_same_time
	self.last_pop_msg_time = 0

	self.__update_handle = BindCallback(self, Message.Update)	--生成一个listener句柄
